...
def default_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ...
      # Physical bridge shape. "box" preserves the original flat rectangular
      # deck. "round" uses a cylinder along x with diameter =
      # 2 * bridge_half_width and top tangent flush with the platform surface.
      bridge_shape="box",
...
class BridgeCrossing(go1_base.Go1Env):
  ...
  def __init__(
    ...
    if self._config.bridge_shape == "box":
      ...
      # The XML bridge is compiled as an unrotated box, so MuJoCo marks its
      # frame as body-aligned. Clear that optimization after changing geom_quat
      # or the cylinder remains upright in data.geom_xmat/rendering/collision.
