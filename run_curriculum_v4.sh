#!/bin/bash
# v4 curriculum: contact-gated frontier + gait shaping (trot-first design)
#
# Key changes from v3:
#   - entropy_cost=0.01 kept through ALL stages (not just stage 0)
#   - Same 5 physical stages as v3; midpoint at 0.32m pre-inserted
#     (between 0.4m and 0.24m where v3 hard-failed)
#
...
#
# Usage:
#   ./run_curriculum_v4.sh                              # fresh run
#   SKIP_TO_STAGE=3 INIT_CKPT=<path> ./run_curriculum_v4.sh
...
WANDB_PROJECT="bridge_crossing_v4"
WANDB_GROUP="${WANDB_GROUP:-v4_gait_fix}"
...
run_stage() {
  ...
  local ckpt_arg="${6:-}"
  local extra_flags="${7:-}"
  local logfile="/tmp/bridge_v4_stage${stage}.log"
  ...
  train-jax-ppo --env_name Go1BridgeCrossing \
    ...
    --wandb_run_name "v4_stage${stage}_${width_str}" \
...
gate_advance() {
  ...
  if [ "$pass" -eq 1 ]; then
    ...
    echo "  Re-invoke: SKIP_TO_STAGE=${stage} INIT_CKPT=${LAST_GOOD_CKPT} bash run_curriculum_v4.sh" >&2
